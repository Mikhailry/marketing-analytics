...
# Compute transition matrix by merging data for 2014 and 2015
...
# Divide each row by its sum to see the percentage of transitions
...
for (i in 2:11) {
  # each year segments membership = previous year membership * transition matrix 
  segments[, i] = segments[, i-1] %*% transition
}

# Plot inactive, cold customers over time
barplot(segments[1, ])
barplot(segments[2, ])
...
# Compute yearly revenue (per column)
